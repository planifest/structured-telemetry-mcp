...
| R-013 | compliance | The auth reversal ships without ADR-032, leaving `component.yml`'s documented "no auth model required" position contradicting the code, in breach of `breakingChangePolicy: requires-adr` | low | low | **Resolved at P2**: ADR-032 accepted 2026-08-08, `component.yml`'s exceptions block revised to cite it. Residual risk is narrow — P3 must land req-001/req-002 before the manifest's revised claim becomes true of the running code, which the manifest itself now states explicitly |
