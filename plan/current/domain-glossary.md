...
| **MAX_LIMIT** | The existing `event_log` row cap (1000), and the ceiling this feature applies to newly bounded modes |
...
| **Tautological test** | An assertion that cannot fail while the code compiles — for example asserting a typed constant array contains its own declared members. Present in `column-allow-list.test.ts` today; removed by req-009 |
