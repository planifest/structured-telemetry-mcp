...
| **MAX_LIMIT** | Not one number. Module-local constants, none exported: `src/query/event-log.ts:19` is 1000, `src/query/distinct-values.ts:20` is 20; `failure_sequence`/`drill_down` have no ceiling until this feature adds 1000 (req-007). **In the running code today, exceeding a ceiling is inconsistent across modes**: `event_log` rejects (`event-log.ts:40-41` throws, `docs/usage-guide.md:667` documents it), but `distinct_values` silently **clamps** (`distinct-values.ts:39`). req-005 makes rejection uniform across every mode — this is a disclosed, deliberate behaviour change to `distinct_values`, not yet true of the code as it stands |
